...
import { Download, RotateCcw, Clock, Users, MessageCircle, ChevronDown, ChevronRight, Star, GitFork, Calendar } from 'lucide-react'
import type { RunOfShow, PresentationConfig } from '@/app/page'
...
export function RunOfShowDisplay({ runOfShow, repoUrl, config, metadata, onReset }: RunOfShowDisplayProps) {
  ...
  const expandAllSections = () => {
    setExpandedSections(new Set(runOfShow.sections.map((_, index) => index)))
  }
...
## Potential Q&A
${runOfShow.qaPredictions.map(q => `- **Q:** ${q}`).join('\n')}
` : ''}

${runOfShow.techQuestions ? `
## Technical Questions to Prepare For
${runOfShow.techQuestions.map(q => `- **Q:** ${q}`).join('\n')}
` : ''}
...
---
*Generated by [Repo to Runway](https://repo-to-runway.vercel.app) - Transform your repositories into compelling presentations*`

    const blob = new Blob([markdown], { type: 'text/markdown' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `${runOfShow.title.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-runofshow.md`
    ...
    <div className="max-w-4xl mx-auto animate-fade-in">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
              ...
              <div className="flex items-center">
                ...
                {config.timeConstraint} ({getTotalDuration()} min content)
              </div>
              <div className="flex items-center">
                <MessageCircle className="h-4 w-4 mr-1" />
                {config.includeQA ? 'Q&A included' : 'No Q&A'}
              </div>
              {metadata?.stars && (
                <div className="flex items-center">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-gray-900">
          Presentation Structure ({runOfShow.sections.length} sections)
        </h2>
                    ...
                    {section.presenterNotes.length > 0 && (
                      <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                        ...
                        <ul className="space-y-2">
                          {section.presenterNotes.map((note, noteIndex) => (
                            <li key={noteIndex} className="text-yellow-700 text-sm flex items-start">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-slide-up">
        {runOfShow.qaPredictions && runOfShow.qaPredictions.length > 0 && (
          <div className="card p-6">
            ...
            <ul className="space-y-3">
              {runOfShow.qaPredictions.map((question, index) => (
                <li key={index} className="text-gray-600 text-sm border-l-2 border-primary-200 pl-3">
                  <span className="text-primary-600 font-medium">Q:</span> {question}
                </li>
              ))}
            </ul>
          </div>
        ...
        {runOfShow.techQuestions && runOfShow.techQuestions.length > 0 && (
          <div className="card p-6">
            <h3 className="font-semibold text-gray-900 mb-4 flex items-center">
              🔧 Technical Questions
            </h3>
            <ul className="space-y-3">
              {runOfShow.techQuestions.map((question, index) => (
                <li key={index} className="text-gray-600 text-sm border-l-2 border-gray-200 pl-3">
                  <span className="text-gray-500 font-medium">Q:</span> {question}
                </li>
              ))}
            </ul>
          </div>
